...
void Application::Start() {
    ...
    // 清除所有已保存的WiFi配置，然后添加默认WiFi凭据
    auto& ssid_manager = SsidManager::GetInstance();
    ssid_manager.Clear();  // 清除flash中所有WiFi配置
    
    auto& wifi_station = WifiStation::GetInstance();
    wifi_station.AddAuth("DYBN", "dy12345678");  // 添加默认WiFi
    ESP_LOGI(TAG, "Cleared all WiFi configs and added default: SSID=DYBN");
    ...
    if (protocol_started) {
    // 启用WiFi信号强度监控
    wifi_monitoring_enabled_ = true;
    ESP_LOGI(TAG, "WiFi signal monitoring enabled after server connection");
    }
